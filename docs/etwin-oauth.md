...
- Client ID: Displayed when starting `eternaltwin`: `Id` or `Key` at your choice (`Key` is recommended)
...
This step is achieved through the `eternaltwin.toml` configuration file (there is no web interface _yet_).

You must add an [OAuth client section](https://gitlab.com/eternaltwin/etwin/-/blob/master/eternaltwin.toml.example#L34)
...
libraries to help you acquire the access token.
...
| Name                      | Value                                                                                                                                                                                                                         |
|---------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Origin                    | Eternaltwin `external_uri` config. `http://localhost:50320/` by default during development, `https://eternaltwin.org/` in production. Your app should get this value from the environment (config file, environment variable) |
...
| `state` parameter         | A string holding your application state. It is recommended to use a signed JWT                                                                                                                                                |
...
| `Authorization` header          | Scheme: `Basic`, login: OAuth client `Id` or `Key`, password: `secret` field from the `eternaltwin.toml` |
